...
impl PerceptionRouter {
    ...
    fn record_escalation(&self, reason: &str, served_by: Tier) {
        ...
        if let Ok(mut v) = self.escalations.lock() {
            v.push(record);
        }
    }
    ...
    pub fn escalations(&self) -> Vec<Escalation> {
        self.escalations.lock().map(|v| v.clone()).unwrap_or_default()
    }
}
...
/// What fraction of `incoming`'s lines the `block` already contains, in `[0, 1]`.
///
...
/// a capture stays one screenful, so the symmetric score decays mechanically as
/// the document accumulates even when each capture overlaps its tail perfectly:
///
/// | sample | block size | common | symmetric | coverage |
/// |---|---|---|---|---|
/// | 2nd | 10 | 8 | 0.80 | 0.80 |
/// | 4th | 14 | 8 | 0.67 | 0.80 |
/// | 5th | 16 | 8 | **0.62 — splits** | 0.80 |
///
/// So a symmetric metric guarantees that every long document eventually
/// fragments, and the threshold only decides when. The question that actually
/// matters is "is this capture material I already have?", which is coverage of
/// the INCOMING text, and it stays stable however large the block gets.
///
...
pub fn coverage(block: &str, incoming: &str) -> f64 {
    let lb: Vec<&str> = block.lines().map(str::trim).filter(|l| !l.is_empty()).collect();
    let li: Vec<&str> = incoming.lines().map(str::trim).filter(|l| !l.is_empty()).collect();
    if li.is_empty() {
        return 0.0;
    }
    let have: std::collections::HashSet<&str> = lb.into_iter().collect();
    let common = li.iter().filter(|l| have.contains(*l)).count();
    common as f64 / li.len() as f64
}

/// Merge `incoming` into `existing`, keeping every unique line in order.
///
/// Lines already present are not repeated; new lines are appended in the order
/// they arrived. This is what turns N overlapping captures of a scrolling pane
/// into one growing document instead of N near-duplicates.
pub fn diff_merge(existing: &str, incoming: &str) -> String {
    ...
    for line in incoming.lines() {
        if !seen.contains(line.trim()) && !line.trim().is_empty() {
    ...
    pub fn absorb(&mut self, text: &str) -> usize {
        ...
        match best {
            ...
            Some((i, score)) if score >= SAME_BLOCK => {
                self.blocks[i] = diff_merge(&self.blocks[i], text);
                i
            }
            _ => {
                self.blocks.push(text.to_string());
...
mod tests {
    ...
    fn router() -> (PerceptionRouter, Arc<AtomicUsize>, Arc<AtomicUsize>) {
        ...
        let router = PerceptionRouter::new(
            Arc::new(CountingProvider { name: "text-tier", calls: t.clone() }),
            Arc::new(CountingProvider { name: "reason-tier", calls: r.clone() }),
            1_000,
        );
        (router, t, r)
    }
    ...
    fn the_comparison_history_is_bounded() {
        ...
        agg.absorb(first);
        assert!(
            agg.blocks().len() > HISTORY,
            "comparison must not scan the whole day: {} blocks",
            agg.blocks().len()
        );
    }
    ...
    #[test]
    fn diff_merge_preserves_the_union_without_duplication() {
        ...
        let merged = diff_merge("a\nb\nc", "b\nc\nd\ne");
        assert_eq!(merged, "a\nb\nc\nd\ne");
        assert_eq!(diff_merge("a\nb", "a\nb"), "a\nb", "a full repeat adds nothing");
