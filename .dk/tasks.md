...
- [x] T018 [S] [US1] Flesh `src/dayflow/engine.rs` — SCOPE NOTE (review I1): `DayflowRun` provides start/stop/status/cap over one shared pipeline, but NOT under the literal names `start_session`/`stop_session`; a capture loop wiring it to real hardware is Wave 6+ work, so these types are not yet called from a running binary. — `start_session` / `stop_session` / `status` over one shared pipeline, honouring the session max-duration cap and closing the in-progress segment on stop (FR-003/005).
...
- [x] T019 [S] [US1] Flesh `src/dayflow/daemon.rs` — SCOPE NOTE (review I1): durable state, resume decision and anomaly reporting exist and are tested, but `daemon.rs` is still an ISLAND — nothing outside its own module calls it. Continuous supervision arrives when the capture loop lands. — continuous supervision, persisted lifecycle state, clean stop, and a restart that resumes onto the same day.
...
- [x] T027 [US3] Crop before extract (was T301) — **PRODUCER OWED (review Q1):** the ladder consumes a `<sample>.regions.json` sidecar and is guarded, but NOTHING WRITES IT YET. The capture loop owes it. Because the path fails OPEN, its absence is invisible — every test passes and T027's whole benefit is silently absent — so `SegmentLatency::samples_read_whole` counts the degradation and the capture-loop task must name this file as a deliverable. — feed the text tier full-resolution region crops from the region cascade, never a downscaled full frame (FR-011). Reuse the existing target/measure path; no new detection code.
...
- [x] T029 [US3] Residency policy (was T331) — **CHANNEL OWED (review):** `ResidencyPolicy::keep_alive` computes the right value against the right cadence, but `VisionProvider::analyze_image(path, prompt)` has no parameter to carry it, so `Resident` is a policy the running system still cannot express. Sending it needs a provider-level change; until then the effective policy is always `OnDemand`. — keep the text tier warm while a recording is active, using the unload window measured in T006; record per-segment latency including any reload (FR-008/013). Three-valued knob: resident / on-demand / off.
